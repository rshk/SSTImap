...
from .clis import MultilineShell, Shell
...
class InteractiveShell(cmd.Cmd):
    ...
    def default(self, line):
        log.log(
            22,
            f"Invalid interactive command: {line.split(' ', 1)[0].lower()}. "
...
Exploitation:
  ...
  down, download [REMOTE] [LOCAL]         Download REMOTE to LOCAL files""",
    ...
    def do_options(self, line):
        ...
        log.log(
            23,
            f"Current SSTImap {self.sstimap_options['version']} interactive mode options:",
        )
        if (
            not self.sstimap_options["url"]
            and not self.sstimap_options["loaded_urls"]
        ...
        if self.sstimap_options["data"]:
            ...
            if self.sstimap_options["data_params"]:
                params = "\n    ".join(
                    [
                        f"{x}: {self.sstimap_options['data_params'][x]}"
        ...
        if self.sstimap_options["force_level"]:
            ...
            log.log(
                26, f"Forced context level: {self.sstimap_options['force_level'][1]}"
            )
        else:
            log.log(26, f"Level: {self.sstimap_options['level']}")
        log.log(
            26,
            f"Engine: {self.sstimap_options['engine'] if self.sstimap_options['engine'] else '*'}"
        ...
        if self.sstimap_options["crawl_depth"] > 0:
            ...
            if self.sstimap_options["crawl_exclude"]:
                log.log(
                    26,
                    f'Crawler exclude RegEx: "{self.sstimap_options["crawl_exclude"]}"',
                )
            log.log(
                26,
                f"Crawl other domains: {crawl_domains.get(self.sstimap_options['crawl_domains'].upper())}",
            )
        else:
        ...
        if "B" in self.sstimap_options["technique"]:
            if self.sstimap_options["boolean_regex_ok"]:
                ...
                log.log(
                    26,
                    f"Boolean error-based blind RegEx: {self.sstimap_options['boolean_regex_ok']}",
                )
            elif self.sstimap_options["boolean_regex_err"]:
                log.log(26, f"Boolean error-based blind detection: RegEx (Error page)")
                log.log(
                    26,
                    f"Boolean error-based blind RegEx: {self.sstimap_options['boolean_regex_err']}",
                )
            else:
                log.log(26, f"Boolean error-based blind detection: Match pages")
                if self.sstimap_options["boolean_match"] not in ["", "*", "all"]:
                    match_params = "\n    ".join(
                        self.sstimap_options["boolean_match"].split(",")
                    )
                    log.log(
                        26,
                        f"Boolean error-based blind matching params:\n    {match_params}",
                    )
                else:
                    log.log(26, f"Boolean error-based blind matching params: all")
                log.log(
                    26,
                    f"Boolean minimum usable matching params: {self.sstimap_options['boolean_match_min']}",
                )
                log.log(
                    26,
                    f"Boolean maximum stable fuzzy matching deviation: {self.sstimap_options['boolean_fuzzy'][0]}",
                )
                log.log(
                    26,
                    f"Boolean minimum error fuzzy matching deviation: {self.sstimap_options['boolean_fuzzy'][1]}",
                )
                log.log(
                    26,
                    f"Boolean matching normal page samples: {self.sstimap_options['boolean_samples'][0]}",
                )
                log.log(
                    26,
                    f"Boolean matching sample size: {self.sstimap_options['boolean_samples'][1]}-"
                    f"{self.sstimap_options['boolean_samples'][2]}",
                )
        if "T" in self.sstimap_options["technique"]:
            log.log(
                26,
                f"Time-based blind detection delay: {self.sstimap_options['time_based_blind_delay']}",
            )
            log.log(
                26,
                f"Time-based verification and exploitation delay: {self.sstimap_options['time_based_verify_blind_delay']}",
            )
    ...
    def do_info(self, line):
        ...
        if not self.checked:
            log.log(
                25, "Target URL was not checked for SSTI. Use 'run' or 'check' first."
            )
            return
    ...
    def do_url(self, line):
        ...
        if not (
            self.sstimap_options["loaded_forms"] or self.sstimap_options["loaded_urls"]
        ):
    ...
    def do_load_urls(self, line):
        if line:
            ...
            if os.path.exists(line):
                try:
                    with open(line, "r") as stream:
                        self.sstimap_options["loaded_urls"] = set(
                            [x.strip() for x in stream.readlines()]
                        )
                    log.log(
                        21,
                        f"Loaded {len(self.sstimap_options['loaded_urls'])} URL(s) from file: {line}",
                    )
                    if not self.sstimap_options["loaded_forms"]:
                        self.set_module(
                            f"\033[31m{len(self.sstimap_options['loaded_urls'])} URLs\033[0m"
                        )
                    self.checked = False
                except Exception as e:
                    log.log(
                        22, f"Error occurred while loading URLs from file:\n{repr(e)}"
                    )
                return
            ...
            if not self.sstimap_options["loaded_forms"]:
                self.set_module(
                    f"\033[31m{parse.urlparse(self.sstimap_options['url']).netloc}"
                    f"\033[0m"
                    if self.sstimap_options["url"]
                    else ""
                )
    ...
    def do_load_forms(self, line):
        if line:
            ...
            if os.path.exists(line):
                try:
                    with open(line, "r") as stream:
                        self.sstimap_options["loaded_forms"] = set(
                            [tuple(x) for x in json.load(stream)]
                        )
                    log.log(
                        21,
                        f"Loaded {len(self.sstimap_options['loaded_forms'])} forms from file: {line}",
                    )
                    self.set_module(
                        f"\033[31m{len(self.sstimap_options['loaded_forms'])} forms\033[0m"
                    )
                    self.checked = False
                except Exception as e:
                    log.log(
                        22, f"Error occurred while loading forms from file:\n{repr(e)}"
                    )
                return
        ...
        if line:
            if self.sstimap_options.get("crawled_forms", None):
                ...
                try:
                    with open(line, "w") as stream:
                        json.dump(
                            [x for x in self.sstimap_options["crawled_forms"]],
                            stream,
                            indent=4,
                        )
                    ...
                    log.log(
                        22, f"Error occurred while saving forms to file:\n{repr(e)}"
                    )
            else:
    ...
    def do_run(self, line):
        ...
        if not (
            self.sstimap_options["url"]
            or self.sstimap_options["loaded_urls"]
            or self.sstimap_options["loaded_forms"]
        ):
            log.log(22, "Target URL cannot be empty.")
            return
        try:
            self.current_plugin, self.channel = checks.scan_website(
                self.sstimap_options
            )
        ...
        self.sstimap_options["loaded_forms"] = None
        self.set_module(
            f"\033[3{'2' if self.checked else '1'}m"
    ...
    def do_data(self, line):
        ...
        if (
            (command[0] == "remove" or command[0] == "rm")
            and len(command) == 2
            and command[1] != ""
        ):
    ...
    def do_force_level(self, line):
        ...
        if line == "":
            log.log(
                24,
                f"Disabling forced template escaping level and language context level",
            )
        ...
        log.log(
            24,
            f"Forcing template escaping level {force_level[0]} and language context level {force_level[1]}",
        )
    ...
    def do_technique(self, line):
        ...
        if technique == "":
            log.log(
                22,
                "Invalid TECHNIQUE value. It must contain at least one of 'R', 'E', 'B' or 'T'.",
            )
            return
        if line != "":
            log.log(
                22,
                "Invalid TECHNIQUE value. It must only contain 'R', 'E', 'B' and 'T'.",
            )
            return
    ...
    def do_bool_ok(self, line):
        ...
        if line:
            log.log(
                24, f'Boolean error-based blind normal page RegEx is set to "{line}".'
            )
        else:
        ...
        if self.sstimap_options["boolean_regex_ok"]:
            ...
            log.log(
                29 if line else 23,
                "Boolean error-based blind detection: RegEx (Error page)",
            )
        else:
            log.log(
                29 if line else 23, "Boolean error-based blind detection: Match pages"
            )
    ...
    def do_bool_err(self, line):
        ...
        if line:
            log.log(
                24, f'Boolean error-based blind error page RegEx is set to "{line}".'
            )
        else:
        ...
        if self.sstimap_options["boolean_regex_ok"]:
            log.log(
                29 if line else 23,
                "Boolean error-based blind detection: RegEx (Normal page)",
            )
        elif self.sstimap_options["boolean_regex_err"]:
    ...
    def do_bool_match(self, line):
        if line not in ["", "*", "all"]:
            ...
            log.log(
                24, f"Boolean error-based blind matching params:\n    {match_params}"
            )
        else:
    ...
    def do_bool_fuzzy(self, line):
        ...
        log.log(
            24,
            f"Fuzzy matching allows deviation of {boolean_fuzzy[0]} for stable params",
        )
        log.log(
            24,
            f"Fuzzy matching requires deviation of {boolean_fuzzy[1]} to detect error",
        )
        if self.sstimap_options["boolean_regex_ok"]:
            log.log(29, "Boolean error-based blind detection: RegEx (Normal page)")
        elif self.sstimap_options["boolean_regex_err"]:
    ...
    def do_bool_samples(self, line):
        ...
        log.log(
            24,
            f"Matcher will try {boolean_samples[0]} payloads of {boolean_samples[0]}-{boolean_samples[0]}"
    ...
    def do_blind_delay(self, line):
        ...
        log.log(
            24,
            f"Delay for time-based blind injection detection is set to {self.sstimap_options['time_based_blind_delay']}",
        )
    ...
    def do_verify_delay(self, line):
        ...
        log.log(
            24,
            f"Delay for time-based blind injection verification and exploitation is set to {self.sstimap_options['time_based_verify_blind_delay']}",
        )
    ...
    def do_generic(self, line):
        ...
        log.log(
            24,
            f"{'En' if overwrite else 'Dis'}abled dedicated plugins for generic template engines",
        )
    ...
    def do_tpl_shell(self, line):
        ...
        if not self.checked:
            log.log(
                22, "Target URL was not checked for SSTI. Use 'run' or 'check' first."
            )
            return
        if self.channel.data.get("engine"):
            if self.channel.data.get("blind") or self.channel.data.get("boolean"):
                log.log(
                    23,
                    "Only blind execution has been found. "
            ...
            log.log(
                21,
                "Inject multi-line template code. Press ctrl-D or type 'EOF' on a new line to send the lines",
            )
            try:
                MultilineShell(
                    call, f"{self.channel.data.get('engine', '')} > "
                ).cmdloop()
            except (KeyboardInterrupt, EOFError):
                print()
            ...
            log.log(
                22, "No code evaluation capabilities have been detected on the target"
            )
    ...
    def do_tpl_code(self, line):
        ...
        if self.channel.data.get("engine"):
            ...
            log.log(
                22,
                "No template code evaluation capabilities have been detected on the target",
            )
    ...
    def do_eval_shell(self, line):
        ...
        if self.channel.data.get("evaluate_blind"):
            ...
            log.log(
                21,
                "Inject multi-line template base language code. "
                "Press ctrl-D or type 'EOF' on a new line to send the lines",
            )
            try:
                MultilineShell(
                    self.current_plugin.evaluate_blind,
                    f"{self.channel.data.get('language', '')} > ",
                ).cmdloop()
            except (KeyboardInterrupt, EOFError):
                print()
            ...
            log.log(
                22,
                "No language code evaluation capabilities have been detected on the target",
            )
    ...
    def do_os_shell(self, line):
        ...
        if self.channel.data.get("execute_blind"):
            ...
            if self.channel.data.get("boolean"):
                log.log(
                    26,
                    "True or False is returned whether the command returns successfully or not.",
                )
            else:
                log.log(
                    26,
                    "Delay is introduced appending '&& sleep <delay>' to the shell commands. "
                    "True or False is returned whether it returns successfully or not.",
                )
            log.log(21, "Run commands on the operating system.")
            try:
                Shell(
                    self.current_plugin.execute_blind,
                    f"{self.channel.data.get('os', 'undetected')} (blind) $ ",
                ).cmdloop()
            except (KeyboardInterrupt, EOFError):
                print()
            ...
            try:
                Shell(
                    ...
                    f"{self.channel.data.get('os', 'undetected')} $ ",
                ).cmdloop()
            except (KeyboardInterrupt, EOFError):
                print()
                log.log(26, "Exiting OS shell")
    ...
    def do_bind_shell(self, line):
        ...
        if self.channel.data.get("bind_shell"):
            ...
            for idx, thread in enumerate(
                self.current_plugin.bind_shell(
                    port, shell=self.channel.args.get("remote_shell")
                )
            ):
                log.log(
                    26, f"Spawn a shell on remote port {port} with payload {idx + 1}"
                )
            ...
            log.log(
                22, "No TCP shell opening capabilities have been detected on the target"
            )
    ...
    def do_reverse_shell(self, line):
        ...
        if self.channel.data.get("reverse_shell"):
            self.current_plugin.reverse_shell(
                host, port, shell=self.channel.args.get("remote_shell")
            )
            try:
            ...
            log.log(
                22, "No reverse TCP shell capabilities have been detected on the target"
            )
    ...
    def do_download(self, line):
        ...
        if self.channel.data.get("read"):
            ...
            log.log(
                22, "No file download capabilities have been detected on the target"
            )
    ...
    def do_reload_modules(self, line):
        ...
        from .plugin import unload_plugins

        unload_plugins()
        load_plugins()
        from .plugin import loaded_plugins

        log.log(
            23,
            f"Reloaded plugins by categories: {'; '.join([f'{x}: {len(loaded_plugins[x])}' for x in loaded_plugins])}",
        )
        from ..cli import load_data_types
        from .data_type import unload_data_types

        unload_data_types()
        load_data_types()
        from .data_type import loaded_data_types
