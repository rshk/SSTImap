...
class Channel:
    def __init__(self, args):
        ...
        self.data_type = loaded_data_types.get(
            self.args.get("data_type", "form").lower(), loaded_data_types["form"]
        )(self.args, self.tag)
        self.get_params = {}
        self.post_params = ""
        self.header_params = {}
        ...
        if cookies:
            ...
            for cookie in splitted_cookies:
                ...
                if self.tag in param:
                    self.injs.append(
                        {"field": "Cookie", "part": "param", "param": param}
                    )
    ...
    def _parse_header(self, all_injectable=False):
        ...
        for param_value in headers:
            ...
            if param.lower() == "cookie":
                ...
                if self.tag in param:
                    self.injs.append(
                        {"field": "Header", "part": "param", "param": param}
                    )
                if self.tag in value or all_injectable:
                    self.injs.append(
                        {
    ...
    def _parse_get(self, all_injectable=False):
        params_dict_list = parse.parse_qs(
            parse.urlsplit(self.url).query, keep_blank_values=True
        )
        ...
        if inj["field"] == "Path":
            ...
            if inj.get("part") == "value":
                if self.tag in cookie_params[inj.get("param")]:
                    cookie_params[inj.get("param")] = cookie_params[
                        inj.get("param")
                    ].replace(self.tag, injection)
                else:
